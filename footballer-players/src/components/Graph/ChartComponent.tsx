...
function setUpChart(
  labels: string[],
  dataChart: number[]
): ChartData<"doughnut" | "bar" | "line", number[], unknown> {
  return {
    labels: labels,
    datasets: [
      {
...
function chartToReturned(
  chartLabel: string,
  title: string,
  dataChart: ChartData<"doughnut" | "bar" | "line", number[], unknown>
) {
  if (chartLabel === "doughnut") {
    return (
      <Doughnut
        data={dataChart as ChartData<"doughnut", number[], unknown>}
    ...
    return (
      <Bar
        data={dataChart as ChartData<"bar", number[], unknown>}
        options={optionsBar(title)}
      />
    );
    ...
    return (
      <Line
        data={dataChart as ChartData<"line", number[], unknown>}
        options={optionsBar(title)}
      />
    );
  } else {
